...
class Recognition_Face:
    def __init__(self,root):
        ...
        img_logo11 = Image.open("college_images/img/facialrecognition.png")
        ...
        img_logo = Image.open("college_images/img/facialrecognition.png")
    ...
    def mark_attendace(self,d,k,s,i):
        with open("present.csv","r+",newline="\n") as f:
            ...
            if ((s not in name_List) and (i not in name_List) and(k not in name_List)  and (d not in name_List)):
                ...
                # myjson={'usn':k,'time':dtString,'date':d1}
    ...
    def post_attendance(self,k,i):
        with open("attendance.csv","r+",newline="\n") as f:
            ...
            for line in myDataList:
                entry=line.split(",")
                name_List.append(entry[0])
            if ((k not in name_List) and(i not in name_List)):
                now=datetime.now()
                d1=now.strftime("%d/%m/%Y")
                dtString=now.strftime("%H:%M:%S")
    ...
    def detect_face(self):
        def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                my_cursor.execute("select usn from new_student where id="+str(id))
                k = my_cursor.fetchone()
                k = ''+''.join(k)
                
                

                my_cursor.execute("select vaccinated from new_student where id="+str(id))
                d = my_cursor.fetchone()
                d = ''+''.join(d)

                if confidence>77:
                    cv2.putText(img,f"USN:{k}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,0),2)
                    cv2.putText(img,f"Name:{i}",(x,y-80),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,0),2)
                    cv2.putText(img,f"Department:{s}",(x,y-105),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,0),2)
                    cv2.putText(img,f"Vaccinated:{d}",(x,y-130),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,0),2)
                    # self.mark_attendace(d,k,s,i)
                    self.post_attendance(k,i)
